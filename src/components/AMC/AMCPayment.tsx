...
const AMCPayment = ({ payment, amcId, onClose }: IProps) => {
  ...
  const form = useForm<IPaymentForm>({
    defaultValues: {
      ...
      received_date: payment.received_date
        ...
        : undefined,
      proforma_date: payment.proforma_date
        ? new Date(payment.proforma_date)
        : undefined,
  ...
  const onSubmit = async (data: IPaymentForm) => {
    ...
    if (
      currentStatus === PAYMENT_STATUS_ENUM.PROFORMA ||
      ...
      if (!data.proforma_date) {
        toast({
          ...
          description:
            "proforma date is required when status is proforma or beyond.",
        });
        return;
      }
    }
    ...
    if (
      ...
      if (!data.invoice_date) {
        toast({
          ...
          title: "Validation Error",
          description:
            "Invoice date is required when status is Invoice or beyond.",
        });
        return;
      }
      ...
      if (!data.invoice_document) {
        toast({
          ...
          title: "Validation Error",
          description:
            "Invoice document is required when status is Invoice or beyond.",
        });
        return;
      }
    }
  ...
  const isFieldRequired = (fieldName: string) => {
    ...
    if (fieldName === "invoice_date" || fieldName === "invoice_document") {
      return [PAYMENT_STATUS_ENUM.INVOICE, PAYMENT_STATUS_ENUM.PAID].includes(
        currentStatus,
      );
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        .map(([key, value]: [string, PAYMENT_STATUS_ENUM]) => (
                          ...
                          >
                            <div className="flex items-center">
                              <div
                                className={`w-2 h-2 rounded-full ${paymentStatusColor(value)} mr-2`}
                              ></div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel
                    className={`text-gray-500 ${isFieldRequired("proforma_date") ? 'after:content-["*"] after:text-red-500 after:ml-0.5' : ""}`}
                  >
                    proforma Date
                  </FormLabel>
                  <FormControl>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel
                    className={`text-gray-500 ${isFieldRequired("invoice_document") ? 'after:content-["*"] after:text-red-500 after:ml-0.5' : ""}`}
                  >
                    Invoice Document
                  </FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel
                    className={`text-gray-500 ${isFieldRequired("invoice_date") ? 'after:content-["*"] after:text-red-500 after:ml-0.5' : ""}`}
                  >
                    Invoice Date
                  </FormLabel>
                  <FormControl>
          ...
          <div className="grid grid-cols-1 gap-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel
                    className={`text-gray-500 ${isFieldRequired("received_date") ? 'after:content-["*"] after:text-red-500 after:ml-0.5' : ""}`}
                  >
