...
import { IPurchase, ILicenceObject, ICustomizationObject, IAdditionalServiceObject, PURCHASE_TYPE } from "@/types/order"
...
export function DataTable<TData, TValue>({
  ...
  const renderCustomizations = (customizations: ICustomizationObject[], clientId: string) => {
    ...
    return (
      <div className="pl-6 pt-3 pb-4">
        <div className="flex items-center mb-4">
          ...
          <h4 className="text-base font-medium text-indigo-600">Customizations</h4>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {customizations.map((customization) => (
            <div
              ...
              onClick={() => router.push(`/purchases/${customization._id}?type=${PURCHASE_TYPE.CUSTOMIZATION}&client=${clientId}`)}
            >
              <div className="flex items-center mb-3">
                <FileType2 className="h-4 w-4 mr-2 text-indigo-500" />
                <span className="font-medium text-gray-800">Type: <span className="capitalize">{customization.type}</span></span>
              </div>
              ...
              <div className="flex items-center text-sm text-gray-500 mb-1">
                ...
                <span>Purchase Date: {formatDate(customization.purchased_date)}</span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {licenses.map((license) => (
            <div
              ...
              onClick={() => router.push(`/purchases/${license._id}?type=${PURCHASE_TYPE.LICENSE}&client=${clientId}`)}
            >
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <span className="font-medium">{license.purchase_order_number}</span>
                </div>
                <div>
                  <div className="flex items-center text-gray-500 mb-1">
                    <Calendar className="h-3.5 w-3.5 mr-1.5" />
                  ...
                  <span className="font-medium">{formatDate(license.purchase_date)}</span>
                </div>
                <div>
                  <div className="flex items-center text-gray-500 mb-1">
                ...
                <div>
                  ...
                  <span className="font-medium text-green-600">{formatCurrency(license.rate.amount)}</span>
                </div>
              </div>
            </div>
          ))}
  ...
  const renderAdditionalServices = (services: IAdditionalServiceObject[], clientId: string) => {
    ...
    return (
      <div className="pl-6 pt-3 pb-4">
        <div className="flex items-center mb-4">
          ...
          <h4 className="text-base font-medium text-emerald-600">Additional Services</h4>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {services.map((service) => (
            <div
              ...
              onClick={() => router.push(`/purchases/${service._id}?type=${PURCHASE_TYPE.ADDITIONAL_SERVICE}&client=${clientId}`)}
            >
              <div className="flex items-center mb-3">
              ...
              <div className="bg-emerald-50 p-2 rounded text-sm flex items-center mb-2">
                ...
                <span>
                  Service Period: {formatDate(service.date.start)} - {formatDate(service.date.end)}
                </span>
              </div>
            </div>
          ))}
  ...
  const renderSubItems = (purchase: IPurchase) => {
    ...
    return (
      <div className="py-2">
        {renderCustomizations(purchase.customizations || [], purchase.client_id._id)}
        {renderLicenses(purchase.licenses || [], purchase.client_id._id)}
        {renderAdditionalServices(purchase.additional_services || [], purchase.client_id._id)}
      </div>
    )
  }
            ...
            <>
              {expandedState.map((state) => (
                <React.Fragment key={state.id}>
                  ...
                  >
                    {table.getRowModel().rows.find(row => row.id === state.id)?.getVisibleCells().map((cell) => (
                  ...
                  {state.isExpanded && (
                    <TableRow key={`${state.id}-expanded`} className="bg-gray-50 hover:bg-gray-50">
