                ...
                {data.licenses.map((license: any) => (
                  <div key={license.id} className="mb-4 last:mb-0">
                    ...
                    <div className="flex justify-between items-center font-semibold">
                      ...
                      <span>
                        {formatCurrency(
                          license.total_license * license.rate.amount,
                        )}
                      </span>
                    </div>
                  </div>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span>
                    {data.amc_details.start_date
                      ? formatDate(data.amc_details.start_date.toString())
                      : "Not Provided"}
                  </span>
                </div>
                <div className="flex justify-between items-center">
...
const PageWithQuery = ({ id }: { id: string }) => {
  ...
  const [createOrderApi, { isLoading: isCreateOrderApiLoading }] =
    useCreateOrderMutation();

  const [orderDetailModal, setOrderDetailModal] = useState<{
    open: boolean;
    data: IClientProfitOrderDetail | null;
  }>({ open: false, data: null });
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);

  const router = useRouter();
  const { data: clientProfitData } = useGetProfitFromClientQuery(id, {
    skip: data?.data.orders.length === 0,
  });
  ...
  const handleDeleteClient = async () => {
    try {
      await deleteClientApi(id).unwrap();
      toast({
        variant: "success",
  ...
  const FinacialSummary = (
    <div className="px-5">
      ...
      <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
        <div className="grid grid-cols-2 gap-4">
          <div className="p-3 bg-white rounded-md text-center">
            ...
            <p className="text-gray-600 mb-1 font-normal text-xs">
              (exluding AMC)
            </p>
            <p className="text-2xl text-green-600 font-semibold">
              ₹
              {clientProfitData?.data.total_profit.toLocaleString("en-IN") || 0}
            </p>
            <Dialog>
              <DialogTrigger>
              ...
              <DialogContent className="sm:max-w-[425px]">
                ...
                <RevenueBreakdownTable
                  breakdown={
                    clientProfitData?.data?.revenue_breakdown || {
                      base_cost: 0,
                      customizations: 0,
                      licenses: 0,
          ...
          <div className="p-3 bg-white rounded-md text-center">
            ...
            <p className="text-2xl text-green-600 font-semibold">
              {formatCurrency(clientProfitData?.data.total_amc_collection || 0)}
            </p>
          </div>
        </div>
          ...
          <p className="text-lg text-green-600 font-semibold">
            {formatCurrency(
              (clientProfitData?.data.total_profit || 0) +
                (clientProfitData?.data.total_amc_collection || 0),
            )}
          </p>
        </div>
      </div>
      ...
      <Table className="">
        ...
        <TableBody>
          {clientProfitData?.data.orders.length ? (
            clientProfitData?.data.orders.map((order, index) => (
              <TableRow
                key={index}
              ...
              >
                ...
                <TableCell className="text-center">
                  {order.products.map((prod) => prod.name).join(", ")}
                </TableCell>
                <TableCell className="text-center">
                  {order.licenses?.length}
                </TableCell>
                <TableCell className="text-center">
                ...
                <TableCell className="text-center">
                  {dateToHumanReadable(order.purchased_date)}
                </TableCell>
                <TableCell className={`text-right capitalize `}>
                  <Badge
                    variant={"default"}
                    className={`text-white ${order.status === "active" ? "text-green-400 " : "text-red-400"}`}
                  >
                    {order.status}
                  </Badge>
                </TableCell>
                <TableCell className="text-right">
                  <Link
                    href={`/purchases/${(order as any).id}?type=order&client=${id}`}
                    passHref
                    target="_blank"
                  >
                    <Button variant="ghost" size="icon" className="eye-button">
            ...
            <TableRow className="">
              ...
              >
                No Order Purchased by the client
              </TableCell>
            </TableRow>
          )}
        </TableBody>
      ...
      {orderDetailModal.open && orderDetailModal.data && (
        <OrderDetailModal
          ...
          onOpenChange={(open) =>
            setOrderDetailModal({
              open,
              data: open ? orderDetailModal.data : null,
            })
          }
    ...
    <div>
      ...
      <div className="mt-5 space-y-10">
        <ClientDetail
          ...
          disable
          handler={updateClientDataHandler}
        />
        {!data?.data.orders.length ? (
          <OrderDetail
            isLoading={isCreateOrderApiLoading}
            title="Intial Order"
